...
pop_and_area <- read.csv("C:/PW/TWD/Lab6/pop_and_area.csv")
...
ca %>% 
  ggplot(aes(x = long, y = lat, group = group, fill = people_per_sq_mile)) + 
  coord_map("albers", 25, 50) +
  geom_polygon(color = "black") +
  scale_fill_fermenter(trans = "log10",
                       palette = 10,
                       direction = 1) +
  theme_void()
...
# Zapewnij czytelność przygotowanej mapy.
ca_density <- ca %>% 
  ggplot(aes(x = long, y = lat, group = group, fill = people_per_sq_mile / 1.6)) + 
  coord_map("albers", 25, 50) +
  geom_polygon(color = "black") +
  scale_fill_fermenter(trans = "log10",
                       palette = 10,
...
index_table <- data.frame(region = unique(country$region), index = rnorm(length(unique(country$region)), 0, 5))
country %>%
