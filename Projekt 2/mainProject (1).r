...
# Hubert
dataH <- as.data.frame(read.csv("C:/studia/2rok/TWD/Projekt2/kod/data.csv"))
wordH <- as.data.frame(read.csv("C:/studia/2rok/TWD/Projekt2/kod/words_data.csv"))
seqH <- as.data.frame(read.csv("C:/studia/2rok/TWD/Projekt2/kod/seq_data.csv"))
# Paweł
#dataP <- as.data.frame(read.csv("ścieżka"))
#wordP <- as.data.frame(read.csv("ścieżka"))
#seqP <- as.data.frame(read.csv("ścieżka"))
dataP <- as.data.frame(1)
wordP <- as.data.frame(1)
seqP <- as.data.frame(1)
# Miłosz
#dataM <- as.data.frame(read.csv("ścieżka"))
#wordM <- as.data.frame(read.csv("ścieżka"))
#seqM <- as.data.frame(read.csv("ścieżka"))
dataM <- as.data.frame(1)
wordM <- as.data.frame(1)
seqM <- as.data.frame(1)
...
server <- function(input, output, session) {
  ...
  output$emojicloud <- renderPlot({
    ...
    emoji_df <- word_df[str_detect(word_df$words, emoji_regex), ]
    ...
    emoji_plot <- ggplot(head(emoji_df, 50), aes(label = words, size = count)) +
      geom_text_wordcloud_area(rm_outside = TRUE,
                               layout = "position", grid_size = 50, color = "#FF00FF") +
